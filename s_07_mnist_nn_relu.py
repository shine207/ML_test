# batch_size 100, regularization_strength 0.002, desc learning_rate 0.1, epochs 50 
#    epoch:    49 Loss: 0.144 Acc: 96.84% test accuracy :  0.9705
# batch_size 100, regularization_strength 0.002, adam learning_rate 0.001, epochs 50 
#    epoch:    49   Loss: 0.040 Acc: 99.27% test accuracy :  0.9807
...
#W = tf.Variable(tf.random_uniform([784, 256], -1.,1.), name='weight')
W = tf.get_variable("W1", shape=[784, 1024], initializer=tf.contrib.layers.xavier_initializer())
...
layer2 = tf.nn.relu(tf.matmul(layer1_,W2) + b2)
layer2_ = tf.nn.dropout(layer2, dropout_rate)

W3 = tf.get_variable("W3", shape=[512, 512], initializer=tf.contrib.layers.xavier_initializer())
b3 = tf.Variable(tf.random_normal([512]), name='bias')
layer3 = tf.nn.relu(tf.matmul(layer2_,W3) + b3)
layer3_ = tf.nn.dropout(layer2, dropout_rate)
'''
...
with tf.Session() as sess:
    ...
    writer = tf.summary.FileWriter('./logs/s_06_1')
    ...
    for epoch in range(training_epochs):
        ...
        for i in range(total_batch):
            ...
            acc, c, _ = sess.run([accuracy, cost, optimizer], feed_dict={X: batch_xs, Y: batch_ys, dropout_rate:0.9})
        ...
        s = sess.run(summary, feed_dict={X: batch_xs, Y: batch_ys, dropout_rate:0.9})
